...
# Returns the current syntax highlighting mode.
sub HighlightMode
...
#
sub TextAppVersion
    {                                           #ND+, version mm-dd-yyyy
#   return 'Development Release 12-07-2007 (1.35 base)';
    return 'Development Release 7-18-2013 (1.4 base)';
    };
...
#   return 'http://www.naturaldocs.org';
    return 'http://ndplus.sourceforge.net';     #ND+, version       
    };
...
sub ParseCommandLine
    ...
    while ($index < scalar @ARGV)
        ...
        if (substr($arg, 0, 1) eq '-')
            ...
            elsif ($option eq '-hl')                #ND+, syntax
            	{
            	$valueRef = \$highlightOption;
            	}
            elsif ($option eq '-ag')
                {
    ...
    if (defined $highlightOption)               #ND+, syntax
        {
        $highlightOption = lc($highlightOption);

        if ($highlightOption eq 'off')
           { $highlightMode = 0; }
        elsif ($highlightOption eq 'code')
           { $highlightMode = 1; }
        elsif ($highlightOption eq 'all')
           { $highlightMode = 2; }
        else
           { push @errorMessages, 'Invalid highlight mode "$highlightOption", either "off", "code" or "all"'; }
        }
    else
        { $highlightMode = 1; }
...
sub PrintSyntax
    ...
    . "\n"
    . " -hl [option]\n--highlight [option]\n"
    . "    Syntax highlighting mode, Default=code.\n"
    . "    off  - Disable syntax highlighting.\n"
    . "    code - Enable only with code sections.\n"
    . "    all  - Apply to prototypes and all codes.\n"
    . "\n"
    . " -r\n--rebuild\n"
...
sub LoadAndComparePreviousSettings
    ...
    if (!$fileIsOkay)
        ...
        my $prevHighlightMode = NaturalDocs::BinaryFile->GetUInt8();
        ...
        if ($prevHighlightMode ne $highlightMode)
            {  NaturalDocs::Project->RebuildEverything();  };
...
sub SavePreviousSettings
    ...
    NaturalDocs::BinaryFile->WriteUInt8($self->HighlightMode());
