...
sub BuildPrototype #(type, prototype, sourceFile, file)
    ...
    if ($file && $language->Name() eq 'Text File')
        ...
        if ($languageModeline)
            {
print "Prototype: ".$file->Source()."=$languageModeline\n";
            }
        }
...
sub NDMarkupToHTML #(sourceFile, text, symbol, package, type, using, style, imgSeq)
    {
    my ($self, $sourceFile, $text, $symbol, $package, $type, $using, $style, $imgSeq) = @_;
...
#print "NDMarkupToHTML\n";
#print "  file:$sourceFile\n";
#print "  mode:$highlightMode ($highlightLang)\n";
    ...
    while (scalar @splitText)
        ...
        if ($text =~ /^<code([^>]*)>$/)         #ND+
            ...
            $outputFile =~ s/(.*)\..*/\1/;
            $outputFile .= $diagramAnchorNumber++;
            $outputFile .= ".svg";
            ...
            $text =~ s/<ditaa([^>]+)>(.*?)<\/ditaa>/
                            $self->BuildDitaa($1, $2, $sourceFile, $imgSeq)/gse;

            # mscgen images                     #ND+, mscgen
            $text =~ s/<mscgen([^>]+)>(.*?)<\/mscgen>/
                            $self->BuildMscgen($1, $2, $sourceFile, $imgSeq)/gse;

            # sdedit images                     #ND+, sdedit
            $text =~ s/<sdedit([^>]+)>(.*?)<\/sdedit>/
                            $self->BuildSeqDiagEditor($1, $2, $sourceFile, $imgSeq)/gse;
            ...
            # ND+, indenting (bulleted, ordered and description lists)
            $text =~ s/<indent([0-9]+)>/<div class=CListIndent$1>/g;
            $text =~ s/<\/indent([0-9]+)>/<\/div>/g;
...
#ND+, image/package
#
#   Function:       OLD_BuildImage
#
#       Creates a HTML link to a image, importing and relating the image file with
#       the owning source file as an auxiliary file.
#
#   Parameters:
#
#       img - The img to link to.
...
#       The HTML link, complete with tags.
#
#sub OLD_BuildImage #(img, sourceFile, imgSeq)
#    {
#    my ($self, $img, $sourceFile, $imgSeq) = @_;
#
#    ${$imgSeq}++;                                       # increment file sequence
#
#    if (!NaturalDocs::File->PathIsAbsolute($img))
#        {                                               # make absolute
#        my ($svol, $sdir) = NaturalDocs::File->SplitPath($sourceFile);
#
#        $img = NaturalDocs::File->JoinPaths($svol.$sdir, $img);
#        }
#
#    my ($vol, $dir, $file) = NaturalDocs::File->SplitPath($self->OutputFileOf($sourceFile, 1));
#    my ($infile) = NaturalDocs::File->CanonizePath($img);
#    $file = NaturalDocs::File->JoinPaths($vol.$dir, "$file-$$imgSeq.".NaturalDocs::File->ExtensionOf($img));
#
#    File::Copy::copy($infile, $file);                   # import
#
#                                                        # child support
...
sub BuildDitaa
    {
    my ($self, $options, $img, $sourceFile, $imgSeq) = @_;

    ${$imgSeq}++;                                       # increment file sequence

    my ($vol, $dir, $file) = NaturalDocs::File->SplitPath($self->OutputFileOf($sourceFile, 1));

    my ($infile) = NaturalDocs::File->JoinPaths($vol.$dir, "ditaa-img-$$imgSeq.ditaa");
    my ($outfile) = NaturalDocs::File->JoinPaths($vol.$dir, "ditaa-out-$$imgSeq.log");

    $file = NaturalDocs::File->JoinPaths($vol.$dir, "$file-$$imgSeq.png");
    ...
    close(INFILE);
                                                        ...
                                                        # XXX - Settings()
    my $rv = system("java -jar ./plugin/ditaa.jar $options $infile $file >$outfile");
...
sub BuildMscgen
    ...
    my ($infile) = NaturalDocs::File->JoinPaths($vol.$dir, "mscgen-img-$$imgSeq.msc");
    my ($outfile) = NaturalDocs::File->JoinPaths($vol.$dir, "mscgen-out-$$imgSeq.log");
    ...
    close(INFILE);
                                                        ...
                                                        # XXX - Settings()
    my $rv = system(".\\plugin\\mscgen -T png -i $infile -o $file $options >$outfile");
...
sub BuildSeqDiagEditor
    ...
    my ($infile) = NaturalDocs::File->JoinPaths($vol.$dir, "sdedit-img-$$imgSeq.sd");
    my ($outfile) = NaturalDocs::File->JoinPaths($vol.$dir, "sdedit-out-$$imgSeq.log");
    ...
    close(INFILE);
                                                        # XXX - Settings()
    my $rv = system("java -jar ./plugin/sdedit.jar -o $file -t png -r Portrait $infile >$outfile 2>&1");
