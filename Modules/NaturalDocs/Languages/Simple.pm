...
use NaturalDocs::DefineMembers 'LINE_COMMENT_SYMBOLS', 'LineCommentSymbols()', 'SetLineCommentSymbols() duparrayref',
                                'BLOCK_COMMENT_SYMBOLS', 'BlockCommentSymbols()', 'SetBlockCommentSymbols() duparrayref',
                                'PROTOTYPE_ENDERS',
...
sub ParseFile #(file, topicsList)
    ...
    open(SOURCEFILEHANDLE, '<' . $file->Source())
        or die "Couldn't open input file " . $file->Source() . "\n";
    ...
    if ($self->Name() eq 'Text File')
        ...
        # Unlike commented input, plain text input must be pre-parsed which itergation 
        # to filter out all -ND- specifications.
        $file->ParseModeLines(\@commentLines, 1);

        NaturalDocs::Parser->OnComment($file, \@commentLines, 1);
        }
        ...
        while (defined $line)
            ...
            if (scalar @commentLines)
                {
                $file->ParseModeLines(\@commentLines)
                    if (! defined $file->Modelines());
                ...
                $lastCommentTopicCount = NaturalDocs::Parser->OnComment($file, \@commentLines, $lineNumber);
