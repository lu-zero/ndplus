...
#
sub ParseForCommentsAndTokens #(ParsedFile file, string[] lineCommentSymbols, string[] blockCommentSymbols, string[] javadocLineCommentSymbols, string[] javadocBlockCommentSymbols)
    {
    my ($self, $file, $lineCommentSymbols, $blockCommentSymbols,
    ...
    $file->ParseModeLines(\@lines)              # one-shot
        if (! defined $file->Modelines());

    # Go through the file

    for (my $lineIndex = 0; $lineIndex < scalar @lines;)
        {
        ...
        # In-line comments without trailing tokens

        if ($self->IsInlineComment(\$line, $lineCommentSymbols))
            {                                   #ND+, line-comments
        ...
        if (scalar @commentLines)
            {
            NaturalDocs::Parser->OnComment($file, \@commentLines, $commentLineNumber, $isJavaDoc);
            @commentLines = ( );
            $isJavaDoc = undef;
            };
...
#       Determines if the line starts with any of the passed symbols, plus
#       one of three embedded documentation markers (/<, !< or *<).
#
#   Parameters:
...
#
#   Returns:
#       If the line starts with any of the passed comment symbols, plus a
#       known marker otherwise returns 0.
#
#                                               #ND+
sub IsInlineComment #(lineRef, symbols)
    {
    ...
    if ($index != -1 && substr($$lineRef, 0, $index) =~ /^[ \t]*$/)
        {
        my $line = substr($$lineRef, $index + length($symbol), 16);
...
sub NewScope
    ...
    return NaturalDocs::Languages::Advanced::Scope->New(@_);
    };
...
#   Function: Scope
#
#   Retrieve the current scope record
#ND+
sub Scope
    {
    my $self = shift;
...
#ND+$self->[SCOPE_STACK] = [ NaturalDocs::Languages::Advanced::Scope->New(undef, undef) ];
    $self->[SCOPE_STACK] = [ $self->NewScope(undef, undef) ];
...
sub StartScope #(closingSymbol, lineNumber, package)
    ...
    push @{$self->[SCOPE_STACK]},
            $self->NewScope($closingSymbol, $package, $self->CurrentUsing());
#ND+        NaturalDocs::Languages::Advanced::Scope->New($closingSymbol, $package, $self->CurrentUsing());
