...
use NaturalDocs::DefineMembers 'NAME', 'Name()',
                                    ...
                                    'TABLENGTH', 'TabLength()', 'SetTabLength()',               #ND+, tab&indent
                                    'INDENTLENGTH', 'IndentLength()', 'SetIndentLength()',      #ND+, tab&indent
                                    'IGNORED_PREFIXES',
                                    'ENUM_VALUES';
...
#
sub ParsePrototype #(type, prototype, [isClass])
    {
    my ($self, $type, $prototype, $isClass) = @_;

    if (!defined $isClass)                      #ND+, cpp (allow overloaded class usage)
        {
        $isClass = NaturalDocs::Topics->TypeInfo($type)->ClassHierarchy();
        }
