...
#
use constant MAXFILESINGROUP => 99;                     #ND+, was 6
...
#   String: pagefooter
#
#   The page footer for the documentation.
#
my $pagefooter;
...
sub LoadMenuFile
    ...
    if ($version = NaturalDocs::ConfigFile->Open(NaturalDocs::Project->UserConfigFile('Menu.txt'), 1))
        ...
        while (my ($keyword, $value, $comment) = NaturalDocs::ConfigFile->GetLine())
            ...
            elsif ($keyword eq 'pagefooter')            #ND+, pagefooter
                {
                if (!defined $pagefooter)
                    {  $pagefooter = $value;  }
                else
                    {  $pagefooter .= '<br>'.$value;  }
                }
...
sub SaveMenuFile
    ...
    if (defined $pagefooter)                            #ND+, pagefooter
        {
        my @lines = split( /<br>/, $pagefooter );

        foreach my $line (@lines)
            { 
            print MENUFILEHANDLE 'PageFooter: ' . $line . "\n";
            }
        }
    else
        {
        print MENUFILEHANDLE
        "# You can add a page footer to your documentation like this:\n"
        . "# PageFooter: [text]\n"
        . "# If you want to add a copyright notice, this would be the place to do it.\n";
        };
